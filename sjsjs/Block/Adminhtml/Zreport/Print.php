...
class Magestore_Webpos_Block_Adminhtml_Zreport_Print extends Mage_Adminhtml_Block_Template
{
    ...
    public function getZreportData(){
        ...
        if(empty($theoreticalClosingBalance)){
            $theoreticalClosingBalance = floatval($data['float_amount']) + floatval($data['cash_sale']) + floatval($data['cash_added']) - floatval($data['cash_removed']) - floatval($data['refund_amount']);
        }
        if(empty($difference)){

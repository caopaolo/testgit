...
class Magestore_Webpos_Helper_Config extends Mage_Core_Helper_Abstract
{
    ...
    */
     ...
     * @return string
     */
    public function getDefaultCustomer($storeId = null)
    {
        ...
        // Mark: create default customer by website id
        if ($storeId) {
            $customerIdInStore = Mage::getStoreConfig(self::DEFAULT_CUSTOMER_ID_PATH, $storeId);
            $customerInStore = Mage::getModel('customer/customer')->load($customerIdInStore);
            $store = Mage::getModel('core/store')->load($storeId);
            if ($customerInStore->getId()){
                if ($store->getWebsiteId() == $customerInStore->getWebsiteId()){
                    return $customerInStore;
                } else {
                    $this->duplicateCustomerToNewWebsite($customer, $store->getWebsiteId(), $storeId);
                }
            } else {
    ...
    protected function duplicateCustomerToNewWebsite($customer, $newWebSiteId, $storeId){
        $websiteCustomer = Mage::getModel("customer/customer");
        $websiteCustomer->setWebsiteId($newWebSiteId);
        $websiteCustomer->loadByEmail($customer->getEmail());
        if (!$websiteCustomer->getId()){
            $customer->setId(null)
        ...
        Mage::getConfig()->saveConfig('webpos/guest_checkout/customer_id', $customer->getId(), 'websites', $storeId);
    ...
    {
        $paths = array(
            'webpos/receipt/title_not_created_invoice',
            'webpos/receipt/title',
    ...
    public function generateGuestCustomerAccount()
    {
        ...
        $taxvat = $this->getStoreConfig('webpos/guest_checkout/taxvat');
        $prefix = $this->getStoreConfig('webpos/guest_checkout/prefix');
        $middlename = $this->getStoreConfig('webpos/guest_checkout/middlename');
        $suffix = $this->getStoreConfig('webpos/guest_checkout/suffix');
        $gender = $this->getStoreConfig('webpos/guest_checkout/gender');
        $dob = $this->getStoreConfig('webpos/guest_checkout/dob');
        if ($dob) {
            $format = Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT);
            $date = Mage::app()->getLocale()->date($dob, $format, null, false);
            $dob = $date->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
        }
        ...
        $customerData->setPrefix($prefix);
        $customerData->setMiddlename($middlename);
        $customerData->setTaxvat($taxvat);
        $customerData->setSuffix($suffix);
    ...
    {
        $customerConfig = Mage::getStoreConfig('customer');
        $customer = Mage::getResourceModel('customer/customer');
        $genderOptions = $customer->getAttribute('gender')->getSource()->getAllOptions();
        array_shift($genderOptions);
        $customerConfig['gender_options'] = $genderOptions;
